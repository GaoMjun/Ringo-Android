...
const mediump vec4 params = vec4(0.33, 0.63, 0.4, 0.35);
...
void main() {
    ...
    float sampleColor = texture2D(s_texture, textureCoordinate).g * 22.0;
    ...
    sampleColor += texture2D(s_texture, blurCoordinates[8]).g * 2.0;
    sampleColor += texture2D(s_texture, blurCoordinates[9]).g * 2.0;
    sampleColor += texture2D(s_texture, blurCoordinates[10]).g * 2.0;
    sampleColor += texture2D(s_texture, blurCoordinates[11]).g * 2.0;
